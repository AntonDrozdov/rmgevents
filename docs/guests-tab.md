...
Документ отражает фактическую реализацию на момент написания: роли `Administrator`, `Manager`, `Creator`, строгую статусную модель, групповую область доступа, историю решений, поиск, постраничный вывод и интерфейс со статусом-кнопкой.
...
Над таблицей есть строка поиска и фильтр по статусу. Фильтр содержит вариант «Все статусы» и все рабочие статусы гостя. Поиск и фильтр применяются вместе, результат остаётся постраничным.
...
| `Infrastructure/Services/GuestService.cs` | Бизнес-правила, статусы, permissions, квоты, история, нормализация поиска и пагинации |
| `Infrastructure/Services/PermissionService.cs` | Своя группа/потомки и получение профиля по LoginId/EventId |
| `Infrastructure/Repositories/GuestRepository.cs` | Загрузка Group и Decisions, CRUD, подсчёт квоты, поиск и постраничные запросы |
...
|---|---|---|---|
| `GET /events/{eventId}/guests?page=&pageSize=&search=&status=` | Получить страницу гостей текущего мероприятия и историю; `status` фильтрует по текущему статусу | `[Authorize]` | `PagedResultDto<GuestDto>` |
...
| Файл | Ответственность |
|---|---|
| `frontend/src/pages/GuestsPage.tsx` | Постраничный список, поиск, похожие гости в форме создания, CRUD-модалки, вычисление области групп, статусные действия, progress bar |
| `frontend/src/services/apiClient.ts` | Методы всех guest endpoints, включая постраничный список и поиск похожих |
| `frontend/src/types/index.ts` | `GuestDto`, `GuestSearchResultDto`, `PagedResultDto`, `GuestDecisionDto`, request-типы и union статусов |
...
| `frontend/src/index.css` | Цвета статусов, hover-кнопка, таблица, поиск, пагинация, похожие гости, модалка и progress bar |
...
При первом открытии `loadData` параллельно загружает страницу гостей и дерево групп. При изменении страницы, размера страницы, поискового запроса или фильтра статуса страница гостей загружается повторно. Изменение строки поиска применяется с задержкой 450 мс; если в строке меньше двух символов, `search` на backend не отправляется. При смене статуса frontend возвращается на первую страницу.
...
Сейчас миграции схлопнуты в одну актуальную начальную миграцию `20260901111221_InitialCreate`.

Она сразу создаёт таблицы `guests` и `guest_decisions` в текущем виде: статус гостя имеет длину 30 символов, default `saved`, `created_by_user_id` является обязательным FK на `users.id`, а история решений хранит `actor_user_id`, `actor_name`, `action` и `created_at`. Для отображения таблицы `GuestDto` возвращает ФИО и роль создателя через `createdByName` и `createdByRoleName`.

Отдельных миграций переноса старых статусов и исправления старых `created_by_user_id` в коде больше нет. Если такую историю нужно накатывать поверх уже существующей production-БД, потребуется отдельный разовый migration/script до перехода на схлопнутую схему.
...
8. Реализовать постраничный `GET /guests` с `page`, `pageSize`, `search` и ответом `PagedResultDto<GuestDto>`.
9. Реализовать поиск похожих гостей из других мероприятий для формы создания.
10. Возвращать после мутации полный `GuestDto` с отсортированной историей.
11. На frontend загрузить страницу гостей, группы и профиль текущего сотрудника.
12. Вычислить доступную ветку групп и видимость действий.
13. Реализовать строку поиска, выбор размера страницы и кнопки перехода по страницам.
14. Реализовать статус-кнопку с неизменной шириной и отдельную иконку отклонения.
15. После статусного действия заменять только одну строку.
16. Реализовать форму редактирования, поиск похожих при создании и горизонтальную историю циклов.
17. Проверить каждую роль, каждый статус, соседние группы и дочерние группы.
