...
Логин отображается, но поле заблокировано. Backend-контракт обновления также не содержит логин, поэтому изменить его прямым запросом через этот endpoint нельзя.
...
Фамилия, имя, email, роль и группа обязательны. После сохранения список загружается повторно.
...
| `PUT /events/{eventId}/users/{userId}` | Изменить профиль, роль и группу | `CanCreateUser` | `204` |
...
`UpdateUserRequest` намеренно не содержит ни `login`, ни `loginId`. Обновление профиля и назначение роли/группы выполняются двумя последовательными вызовами сервиса.
...
|---|---|---|
| `Infrastructure/Services/UserService.cs` | `UserService` | Создание/изменение/удаление User, назначение и проверка роли/группы, правило корневой группы Administrator, сброс пароля |
...
| `Infrastructure/Migrations/20260831154110_AddMustChangePassword.cs` | миграция | Добавляет `logins.must_change_password` с default `false` |
...
12. **Возможна неоднозначность числовых ID в PermissionService.** JWT хранит LoginId, а `HasPermissionAsync` сначала пытается найти User по тому же числовому ID и только затем по LoginId + EventId. При совпадении значений ID это потенциально может выбрать неверный профиль.
13. **Владелец мероприятия может не удалиться.** FK владения и связанные сущности могут запрещать удаление даже при нескольких администраторах; отдельное пользовательское сообщение для этого пока не реализовано.
14. **Email обязателен только на фронтенде.** Backend-контракт допускает `null`, поэтому серверную валидацию обязательности ещё следует добавить.
