...
Если мероприятие завершено, вкладка сотрудников работает в режиме просмотра: список доступен, но создание, редактирование, удаление и сброс пароля скрыты на frontend и запрещены на backend до возврата мероприятия в активные.
...
| Роль | Да | Выпадающий список глобальных ролей; отправляется ID роли. `Administrator` выделяется красным |
...
Кнопка **«Использовать»** подставляет в форму логин, ФИО, email, телефон, роль и группу выбранного профиля. Роль сопоставляется с глобальным справочником по названию, группа — со справочником групп текущего мероприятия по названию. Если подходящего названия нет, сохраняется текущее значение поля. После подстановки все поля остаются доступными для редактирования. Если данные взяты из похожего сотрудника и перед сохранением роль повышается до `Administrator`, показывается модальное предупреждение: эта роль даёт полный доступ, включая создание мероприятий.
...
Роль `Administrator` визуально выделяется красным в выпадающих списках создания и редактирования, чтобы пользователь видел повышенные права до сохранения.
...
| `GET /events/{eventId}/users/search` | Поиск существующих сотрудников по логину, ФИО и email | `CanCreateUser` | `UserSearchResultDto[]` |
