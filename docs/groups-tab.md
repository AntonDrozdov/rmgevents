...
- название по умолчанию — `Default Group`;
- квота по умолчанию — `1000`;
...
| `Infrastructure/Migrations/20260828174218_InitialCreate.cs` | миграция | Исходная таблица `groups`, индексы и FK |
...
10. **Числовой идентификатор из SID — LoginId.** `PermissionService` сначала пытается найти User по такому же числовому ID, затем по LoginId + EventId; совпадение ID потенциально может выбрать неверный профиль.
